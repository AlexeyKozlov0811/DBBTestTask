...
class TestBookBorrowAPI:
    ...
    def books(self, session, user):
        ...
        author = Author.create_obj(
            data=AuthorCreate(name="test_author", birth_date=date.today()), session=session
        )
        publisher = Publisher.create_obj(
            data=PublisherCreate(name="test_genre", est_date=date.today()), session=session
        )
        ...
        BookBorrow.create_obj(
            data=BookBorrowCreate(
                ...
                date_to_return=date.today() + timedelta(days=1),
    ...
    def test_borrow_book(self, session, books, auth_urls, client_logged, user):
        ...
        assert (
            data["date_to_return"]
            == (date.today() + timedelta(days=settings.BORROWING_DAYS)).isoformat()
        )
