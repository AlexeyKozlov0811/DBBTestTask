...
async def borrow_book(book_id: int, session: Session = Depends(get_session),
    ...
    if book.in_stock:
        borrow_record = BookBorrow.create_obj(data=BookBorrowCreate(
            book_id=book.id, user_id=current_user.id, date_borrowed=date.today(), date_to_return=date.today() + timedelta(days=settings.BORROWING_DAYS)
        ), session=session)
...
async def return_book(book_id: int, session: Session = Depends(get_session),
                      current_user: UserData = Depends(get_current_user)):
    book = Book.get_obj_or_404(obj_id=book_id, session=session)
    user_active_borrow = list(filter(lambda record: not record.is_returned and record.user_id == current_user.id, book.borrowings))
